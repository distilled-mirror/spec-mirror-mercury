---
updatedAt: 2026-06-22T18:02:11.000Z
---
...
**Update a card.** Adjust the nickname or spending controls. Status changes are handled through the dedicated freeze, unfreeze, and cancel endpoints — not the update endpoint.
...
```json
{
  "components": {
    "schemas": {
      ...
      "NicknameUpdate": {
        "description": "Nickname update action. Omit the field to keep the current nickname, send null or an empty/whitespace-only string to clear it, or send a string to set it.",
        "nullable": true,
        "type": "string"
      },
      "PhysicalCardStatus": {
      ...
      "UpdateCardRequest": {
        "description": " At least one updateable field must be provided;\n requests where every field is the same are rejected.",
        "properties": {
          "nickname": {
            "allOf": [
              {
                "$ref": "#/components/schemas/NicknameUpdate"
              },
              {
                "description": " Updated card nickname. Omit to keep current, send null/empty to clear, send a string to set."
              }
            ]
          },
          "spendLimit": {
            "allOf": [
              ...
              {
                "description": " Updated spending controls for the card. Omit to leave unchanged."
              }
            ],
            "nullable": true
          }
  ...
  "paths": {
    "/cards/{cardId}": {
      "post": {
        "description": "Update a card's nickname or spending limits.",
